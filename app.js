// app.js
// Full file (mock UI + Zendesk live ticket bridge + AI gateway integration)
...
function renderFromAI(ai) {
  AI_CACHE = ai || null;

  // Chips
  const c = ai?.classification || { category: "—", intent: "—", sentiment: "—", priority: "—" };
  ...
  $("ticketSummary").innerHTML = `
    ...
    <pre style="margin-top:8px; border:1px solid #E3E8F2; border-radius:14px; padding:10px; background:#fff; overflow:auto; font-family: var(--mono); font-size: 12px; white-space: pre-wrap;">${escapeHtml(ai?.summary_markdown || "—")}</pre>
  `;

  // ASK ME default (optional)
  ...
  // Seeker (AI)
  const macros = ai?.seeker?.suggested_macros || [];
  const similar = ai?.seeker?.similar_tickets || [];
  $("seeker").innerHTML = `
    <div class="item">
      <div class="item__t">Suggested Macros</div>
      <div class="item__m" style="white-space:pre-wrap;">${escapeHtml(macros.length ? macros.map(x => `• ${x}`).join("\n") : "—")}</div>
      <div class="item__kvs"><span class="kv">Source: AI</span></div>
    </div>
    <div class="item">
      <div class="item__t">Similar Tickets</div>
      <div class="item__m" style="white-space:pre-wrap;">${escapeHtml(similar.length ? similar.map(x => `• ${x}`).join("\n") : "—")}</div>
      <div class="item__kvs"><span class="kv">Source: AI</span></div>
    </div>
  `;

  // Side convo summary (AI)
  ...
  // Opening/Closure cache for buttons
  window.__AI_OPENING = ai?.opening_message || "";
  window.__AI_CLOSURE = ai?.closure_message || "";

  // Draft reply (AI)
  if (ai?.draft_reply) $("draft").value = ai.draft_reply;
}

async function runAIAndPopulateAll() {
  logEvent({ type: "AI", message: "Calling AI gateway…" });
  toast("Calling AI…");

  const ai = await callAI(ACTIVE);
  console.log("AI RESPONSE JSON:", ai);

  renderFromAI(ai);

  logEvent({ type: "AI", message: "AI populated UI fields", meta: { intent: ai?.classification?.intent } });
  toast("AI populated fields");
}
...
// -----------------------------
// Ticket summarization (local fallback)
// -----------------------------
...
function renderTicketSummaryLocalFallback() {
  ...
  logEvent({ type: "AI", message: "Generated ticket summary + classification (local fallback)", meta: { ...c } });
...
function summarizeSideConvoLocalMock() {
  const msgs = SIDE_CONVO_BY_TICKET[ACTIVE_KEY] || [];
  const summary =
    msgs.length
...
// Opening / Closure (local mock)
function openingMessageLocalMock() {
  const text =
...
function closureMessageLocalMock() {
  const text =
`Hi ${ACTIVE.requester.name},
...
// ASK ME (local mock)
function askMeLocalMock(question) {
...
// Reply generator (local mock)
function generateReplyLocalMock(lang) {
...
function shortenDraftLocalMock() {
  const { name } = ACTIVE.requester;
  return `Hi ${name},
...
// Core UI (mock refresh)
function refreshContextMock() {
...
// -----------------------------
// Zendesk Live Ticket Integration (bookmarklet -> bridge -> CustomEvent)
// -----------------------------
...
// IMPORTANT: Generate Summary uses AI
...
$("btnRunSeeker").addEventListener("click", () => {
  if (AI_CACHE?.seeker?.suggested_macros || AI_CACHE?.seeker?.similar_tickets) {
    renderFromAI(AI_CACHE);
    toast("Seeker (AI) shown");
    return;
  }
  runSeekerLocalMock();
});
...
$("btnOpening").addEventListener("click", () => {
  const aiText = (window.__AI_OPENING || "").trim();
  if (aiText) {
    $("openCloseOut").textContent = aiText;
    $("openCloseOut").classList.remove("empty");
    logEvent({ type: "AI", message: "Displayed AI opening message" });
    toast("Opening (AI)");
    return;
  }
  openingMessageLocalMock();
});

$("btnClosure").addEventListener("click", () => {
  const aiText = (window.__AI_CLOSURE || "").trim();
  if (aiText) {
    $("openCloseOut").textContent = aiText;
    $("openCloseOut").classList.remove("empty");
    logEvent({ type: "AI", message: "Displayed AI closure message" });
    toast("Closure (AI)");
    return;
  }
  closureMessageLocalMock();
});

$("btnSummSide").addEventListener("click", () => {
  if (AI_CACHE?.side_convo_summary) {
    toast("Side convo already summarized by AI");
    return;
  }
  summarizeSideConvoLocalMock();
});
...
$("btnAsk").addEventListener("click", () => {
  ...
  // If AI provided a default answer, show it + echo question
  if (AI_CACHE?.ask_me_default) {
    $("qaAnswer").textContent = `${AI_CACHE.ask_me_default}\n\n(Question: ${q})`;
    $("qaAnswer").classList.remove("empty");
    return;
  }

  // Otherwise use local mock
  const ans = askMeLocalMock(q);
...
$("btnGenReply").addEventListener("click", async () => {
  ...
  // Prefer AI draft if available; if not, try AI call; else local mock
  try {
    if (!AI_CACHE?.draft_reply) await runAIAndPopulateAll();
    if (AI_CACHE?.draft_reply) {
      $("draft").value = AI_CACHE.draft_reply;
    } else {
      $("draft").value = generateReplyLocalMock(lang);
    }
  } catch (e) {
    console.error(e);
    $("draft").value = generateReplyLocalMock(lang);
    logEvent({ level:"ERROR", type:"ERROR", message:"AI failed during draft generation; used mock", meta:{ err:String(e.message || e) } });
  }

  logEvent({ type: "AI", message: "Generated draft reply", meta: { requestId, lang } });
  toast(`Draft generated • ${requestId}`);
});

$("btnShorten").addEventListener("click", () => {
  if (!$("draft").value.trim()) return;
  $("draft").value = shortenDraftLocalMock();
...
$("btnMoreFormal").addEventListener("click", () => {
  if (!$("draft").value.trim()) return;
  $("draft").value = formalDraftLocalMock();
